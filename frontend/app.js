...
async function loadBooks() {
    ...
    books.forEach(book => {
        ...
        const inputC =
        'w-full px-3 py-2 border border-gray-300 rounded bg-white text-gray-900 ' +
    ...
    const sessControls = `
    <div class="mt-2">
    <button id="startBtn-${book.id}" onclick="startSession(${book.id})"
    class="px-2 py-1 bg-green-500 text-white rounded">Start Session</button>
    <button id="stopBtn-${book.id}" onclick="stopSession(${book.id})" disabled
    class="px-2 py-1 bg-red-500 text-white rounded">Stop Session</button>
    ...
    ? [1,2,3,4,5].map(r=>{
        ...
        class="${book.rating===r?'scale-125':'opacity-50'} hover:scale-110 transition">${em}</button>`;
    }).join('')
    : '';
    ...
    <label>Status:
    <select id="status-${book.id}" onchange="updateBook(${book.id})" class="${selectC}">
...
// Load sessions & render chart, pace & streak
async function loadSessionsAndRenderOverview() {
    ...
    for (let i=0;; i++){
        ...
        if (daily[ds]>0) streak++; else break;
    }
...
async function addSearchedBook(t,a,g,th){
    await fetch(apiBase,{method:'POST',headers:{'Content-Type':'application/json'},
                body: JSON.stringify({
                    title: unescape(t),
                                     author: unescape(a),
                                     genre: unescape(g),
